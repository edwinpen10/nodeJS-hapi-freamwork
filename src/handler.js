...
const getAllBooksHandler = (request, h) => {
    ...
    if (params.reading) {
        const books = (params.reading === 1) ? db.filter((book) => book.reading === true) : db.filter((book) => book.reading === false);
        books.map((book) => ({
            id: book.id,
            name: book.name,
    ...
    if (params.finished) {
        ...
        const books = (params.finished === 1) ? db.filter((book) => book.finished === true) : db.filter((book) => book.finished === false);
        books.map((book) => ({
            id: book.id,
            name: book.name,
            publisher: book.publisher
